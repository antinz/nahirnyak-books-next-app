...
export default function ChapterPageClient({ id, chapterId, initialData }) {
  ...
  const toggleFootnote = (index) => {
    setExpandedFootnotes((prev) => ({ ...prev, [index]: !prev[index] }));
  };
  ...
  if (!chapterData)
    return <p className="text-center mt-10">Глава не найдена.</p>;
    ...
    <div className="flex min-h-screen relative">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b font-semibold text-lg">
          ...
          <button
            ...
            aria-label="Закрыть список глав"
          >
            ×
          </button>
        </div>
        ...
        <div className="bg-gray-200 py-5 px-5 md:px-12 lg:px-28">
          {!sidebarOpen && (
            <button
              ...
              className="bg-white hidden sm:block p-2 sm:top-12 sm:left-12 fixed rounded-full shadow-lg transition duration-200 cursor-pointer hover:bg-gray-100"
              aria-label="Открыть список глав"
            >
              <Image src={assets.chapter_icon} alt="Список глав" />
            </button>
          )}
          <div className="flex justify-between items-center">
            <Link href="/">
              <Image src={assets.logo} width={150} alt="Логотип" />
            </Link>
          </div>
          <div className="text-center mt-16 mb-8 px-3">
            <h2 className="text-xl sm:text-3xl font-semibold max-w-[700px] mx-auto">
      ...
      >
        <div className="flex-1 flex justify-center">
          ...
          >
            <Image src={assets.chapter_icon} alt="Главы" />
          </button>
        </div>
